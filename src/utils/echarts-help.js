...
export const getDayxAxis = (start, end, format = style) => {
  ...
  result.push(Vue.filter('DateFormat')(sDate, format))
  for (let i = 1; i < distance; i++) {
    let date = new Date(sDate.valueOf() + unit * i)
    result.push(Vue.filter('DateFormat')(date, format))
  }
...
export const getDayxAxisWithX = (start, end, count, format = style) => {
  ...
  while (from > sDate.valueOf()) {
    let date = Vue.filter('DateFormat')(new Date(from), format)
    let s = Vue.filter('DateFormat')(new Date(from - (count - 1) * unit), format)
    result.push(`${s}-${date}`)
