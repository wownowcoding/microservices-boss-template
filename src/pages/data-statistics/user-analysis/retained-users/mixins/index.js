import { getRetainedUser } from '@/api/data-statistics/user-analysis'
...
export default {
  ...
  methods: {
    ...
    handleSearch(params) {
      const { form } = params
      const { startDate, endDate, displayType } = form
    ...
    getStatistcData(params) {
      ...
      getRetainedUser({
        ...
        .then(res => {
          ...
          const { data } = res
          const result = this.getTableDataByClass(data || [])
    ...
    getTableDataByClass(data) {
      return data.map(item => {
        const obj = { ...item }
        const cellClassName = {}
        Object.keys(obj).forEach(key => {
          if (['date', 'newUserNum'].includes(key)) {
            return
          }
          if (obj[key]) {
            cellClassName[key] = 'highlight-table-info-row'
          }
        })
    ...
    _handleDateFormat(date, hasStart = false) {
      if (hasStart) {
        ...
        return this._toDDMM(start).join('/') + '-' + this._toDDMM(end).join('/')
      }
      return this._toDDMM(date).join('/')
    },
    _toDDMM(date) {
