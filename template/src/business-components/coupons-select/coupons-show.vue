...
export default {
  ...
  data() {
    return {
      ...
      infoArr: [
        { name: this.$t('v3_1_1.coupon') + 'ID', key: 'couponNo' },
        { name: this.$t('v2_1.couponName'), key: 'couponName' },
        { name: this.$t('v1.couponType'), key: 'couponType', render: (h, params) => h('div', this.couponMarketingTypeEnum(params.couponType)) },
        { name: this.$t('v1.couponQuota'), key: 'reduceAmountUsd', render: (h, params) => h('div', params.reduceAmountUsd ? this.Util.money(params.reduceAmountUsd) : '') },
        { name: this.$t('v2_1.validity_period'),render: (h, params) => h('div', this.effectiveInfo(params)) },
        {
  ...
  methods: {
    ...
    detail() {
      this.$router.push({
        path: '/wownow-marketing/wownow-marketing/detail',
        query: {
